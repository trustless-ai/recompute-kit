...
Four facts, each independently established: **content identity**, **existence witness** (independent of the
object's self-reported time), **subject binding** (the witness must reference this subject), and
**witnessed precedence** (the witnessed anchor time precedes the witnessed *thread-open* time, itself a
witnessed fact). Three-state verdict (PASS / FAIL:reason / UNVERIFIABLE:reason); no silent green.

## Run

```
# deterministic verdict logic (graded suite — CI-safe, no network). 21 vectors: 3 PASS / 9 FAIL / 9 UNVERIFIABLE
...
- **Is:** a recomputable check that an origination artifact was independently witnessed to predate the
  thread — with the witness bound to its subject, and thread-open witnessed too.
