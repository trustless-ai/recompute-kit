...
| `deils-reveal.v0` — DEILS leg-2 reveal-check (`content_withheld → content_bound → verdict`, or terminal `content_commitment_mismatch`; fail-closed, and a mismatch is *positive evidence*, not an absence) | **independent second checker built from the spec alone, blind** — `Echo-Merlini`, written before the vectors exist; commitment = `sha256(receipts' JCS(content))`; self-test 4/4 on the archetypes (bound / reorder-still-binds / single-byte-flip-mismatches / withheld). Blind byte-diff on landing. *In build — Fede's `/prove` + vectors pending.* | vectors owned by `@babyblueviper1` in [preaction-governance-conformance](https://github.com/babyblueviper1/preaction-governance-conformance) → `examples/deils-leg2/` · spec + checker in the DEILS companion repo (private; opens with leg-2) |
