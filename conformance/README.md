...
| `chronicle_checkpoint_continuity.v0` — pairwise checkpoint continuity (17 base vectors + the 2 order-precedence co-occurrences) | **two independently-authored implementations** — Pavlo's reference (crystal-receipt) · a from-spec rebuild (`TMerlini`, never read `crystal-receipt/src/**`), incl. the root canonicalization reverse-engineered from golden roots — 19/19 agree with pinned `expected`, 22/22 golden roots. | [`chronicle-checkpoint-continuity-v0/`](./chronicle-checkpoint-continuity-v0/) · vectors from crystal-receipt @ `4ee5a8d0` (#98) + [PR #99](https://github.com/pipavlo82/crystal-receipt/pull/99) |
