...
| `chronicle_checkpoint_continuity.v0` — pairwise checkpoint continuity (final 19 normative vectors, incl. both order-precedence cases) | **two independently-authored implementations** — Pavlo's reference (crystal-receipt) · a from-spec rebuild (`TMerlini`, never read `crystal-receipt/src/**`), incl. the root canonicalization reverse-engineered from golden roots — 19/19 agree with pinned `expected`, 22/22 golden roots. | [`chronicle-checkpoint-continuity-v0/`](./chronicle-checkpoint-continuity-v0/) · spec+vectors hash-pinned from crystal-receipt @ `28f0d2be` (vectors SHA-256 `8fa6d533…`) |
